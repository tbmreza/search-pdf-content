...
{
  if (recent[oldchar - 3] == search[0] && recent[oldchar - 2] == search[1] && (recent[oldchar - 1] == ' ' || recent[oldchar - 1] == 0x0d || recent[oldchar - 1] == 0x0a) && (recent[oldchar - 4] == ' ' || recent[oldchar - 4] == 0x0d || recent[oldchar - 4] == 0x0a))
  {
    return true;
  }
  return false;
...
int _tmain(int argc, _TCHAR *argv[])
{
  ...
  FILE *fileo = fopen("C:\\Users\\tubaghan\\search-pdf-content\\cp\\sample.txt", "w");
  if (fileo)
    fclose(fileo);
  fileo = fopen("C:\\Users\\tubaghan\\search-pdf-content\\cp\\sample.txt", "a");

  //Open the PDF source file:
  //C:\Users\tubaghan\search-pdf-content\cp
  FILE *filei = fopen("C:\\Users\\tubaghan\\search-pdf-content\\cp\\sample.pdf", "rb");
